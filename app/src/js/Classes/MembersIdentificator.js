class MembersIdentificator {
    ...
    run() {
        for (let member in this.members) {
            this.members[member].addEventListener('mouseenter', function(){this.onMouseEnter(member)}.bind(this));
            this.members[member].addEventListener('mouseleave', function(){this.onMouseLeave(member)}.bind(this));
        }
    }
}